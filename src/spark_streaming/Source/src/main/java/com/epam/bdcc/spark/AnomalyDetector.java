...
import static com.epam.bdcc.kafka.KafkaHelper.createConsumerStrategy;
...
public class AnomalyDetector implements GlobalConstants {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (!applicationProperties.isEmpty()) {
            ...
            SparkConf streamingConf = new SparkConf().setAppName(appName).setMaster("local[*]");

            JavaStreamingContext jssc = new JavaStreamingContext(streamingConf, batchDuration);
            jssc.checkpoint(checkpointDir);

            JavaInputDStream<ConsumerRecord<String, MonitoringRecord>> directStream =
                    KafkaUtils.createDirectStream(
                        jssc,
                        LocationStrategies.PreferConsistent(),
                        KafkaHelper.createConsumerStrategy(rawTopicName));

            JavaPairDStream<String, MonitoringRecord> pairDStream =
                    directStream
                        .mapToPair(item -> new Tuple2(KafkaHelper.getKey(item.value()), item.value()));

            pairDStream.checkpoint(checkpointInterval);

            pairDStream.mapWithState(StateSpec.function(mappingFunc))
                    .foreachRDD(rdd -> rdd.foreachPartition(iter -> {
                        ...
                        iter.forEachRemaining(element -> {
                            producer.send(new ProducerRecord<>(enrichedTopicName, element));
                        });
